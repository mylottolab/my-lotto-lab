...
//
// ⚠ 2026-08-09: 이 파일(pay/common.js)에서 mll_rt(리프레시 토큰)/mll_usr(유저정보)를
// 저장하지 않고 있었다. common.js가 auth_gate.js보다 먼저 로드/실행되는 페이지
// (ai_input.html, excel_upload.html, camera_qr.html, lotto_input_page_v4.html 등)에서는
// 이 부트스트랩이 먼저 mll_auth/mll_tok 파라미터를 URL에서 지워버리는 바람에, 뒤이어
// 실행되는 auth_gate.js 쪽 부트스트랩(mll_rt까지 제대로 저장하는 버전)이 "처리할 게 없다"고
// 보고 그냥 넘어갔다. 그 결과 도메인을 넘어(GitHub Pages ↔ Render) 이동해서 들어온 사용자는
// 리프레시 토큰이 전혀 저장되지 않아, 액세스 토큰이 만료되면 자동갱신(ensureFreshToken)이
// 항상 실패하고 "로그인 정보가 필요합니다"가 뜨며 재로그인이 필요했던 것 — 이게 실제
// 보고된 문제의 핵심 원인으로 보인다. 이제 다른 파일들과 동일하게 mll_rt/mll_usr도 저장한다.
// =====================================================
...
// ⚠ 2026-08-09: 위와 짝을 이루는 부분 — 여기서 mll_rt/mll_usr를 실어 보내지 않으면
// 위 부트스트랩을 고쳐도 애초에 받을 값 자체가 없다. 두 군데를 함께 고쳐야 한다.
...
// ⚠ 배포 시 주의: 아래 이미지 데이터(MLL.PIG158_DATAURI)는 매우 긴 base64 문자열이라
// 이 패치 파일에는 생략했습니다. 실제 서버 파일(/pay/common.js)에 반영하실 때는
// 이 줄만 원래 파일 그대로 두시고, 이 파일의 나머지 변경 부분만 옮겨서 적용해주세요.
