...
// "조합 개수만큼 그때그때 베팅"이 아니라, "단위를 미리 구매하면 그 단위마다
// 최대 100개까지 조합을 무료로 채울 수 있는" 방식으로 변경. 1단위 = 100P.
// 경마 베팅처럼 마감 전까지 몇 번이든 나눠서 추가 구매 가능.
// 실제로 등록 가능한 칸을 다 채우든 하나도 안 채우든 상관없음 — 베팅액(=배당 계산 기준)은
// "산 단위 수"이지 "채운 조합 개수"가 아님.
// ⚠ 2026-08-13: FFA_MAX_UNITS를 100 → 1,000으로 수정. 프론트(hub_battles.html의
// BATTLE.MAX_UNITS)는 이미 1,000으로 되어 있었는데 여기(서버)만 100으로 남아있어서,
// 실제로는 100단위(=조합 10,000개)에서 막혀버리는 프론트-서버 불일치 버그였음.
// 최종 강제력은 항상 서버에 있으므로, 여기가 진짜 원인이었음.
const FFA_UNIT_PRICE = 100;
const FFA_MAX_UNITS = 1000;
